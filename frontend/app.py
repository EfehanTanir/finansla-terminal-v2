...
# Fix path — works whether run as "streamlit run frontend/app.py" or from inside /app/frontend
...
# ─── Page Router ──────────────────────────────────────────────────────────────
# Import from pages/ directly (same directory as this file)
page_name = page.split("  ")[-1]

if page_name == "DASHBOARD":
    from pages import dashboard; dashboard.render(api_url)
elif page_name == "BIST STOCKS":
    from pages import bist; bist.render(api_url)
elif page_name == "US STOCKS":
    from pages import us_stocks; us_stocks.render(api_url)
elif page_name == "EU STOCKS":
    from pages import eu_stocks; eu_stocks.render(api_url)
elif page_name == "ETF TRACKER":
    from pages import etfs; etfs.render(api_url)
elif page_name == "MACRO & FX":
    from pages import macro; macro.render(api_url)
elif page_name == "NEWS FEED":
    from pages import news; news.render(api_url)
elif page_name == "SHIP TRACKER":
    from pages import ships; ships.render(api_url)
elif page_name == "CARGO PLANES":
    from pages import planes; planes.render(api_url)
elif page_name == "HEATMAP":
    from pages import heatmap; heatmap.render(api_url)
elif page_name == "STOCK ANALYZER":
    from pages import analyzer; analyzer.render(api_url)
